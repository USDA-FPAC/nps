<template>
  <div :class="'fds-field '+ (hasError ? fieldErrorClass : '') +' '+ extraFieldClasses">
    ...
    <span v-if="HELP_MESSAGE" :id="GROUP_ID + '__help'" class="fds-field__help">{{ HELP_MESSAGE }}</span>
    <span v-if="hasError" :id="GROUP_ID + '__error-message'" class="fds-field__message" role="alert">{{ ERROR_MESSAGE }}</span>
  </div>
</template>
<script>
  ...
  setup(props){
    ...
    onMounted(() => {
      if(displayType == 'vertical'){
        ulClass.value = 'fds-form-list'
      }
      if(displayType == 'horizontal'){
        ulClass.value = 'fds-level'
        extraFieldClasses.value = 'fds-field--block'
      }
